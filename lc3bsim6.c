  ...
  /* your code for MEM stage goes here */
    ...
    if(Get_DATA_SIZE(PS.MEM_CS) == 0){
        if(PS.MEM_ADDRESS & 0x1){
            wdata = PS.MEM_ALU_RESULT & 0x00FF << 8;
        }
        else{
            wdata = PS.MEM_ALU_RESULT & 0x00FF;
        }
    }
    else{
        wdata = Low16bits(PS.MEM_ALU_RESULT);
    }
    ...
    //mem enable and access
    int read_word;
    int dcache_r;
    if(Get_DCACHE_EN(PS.MEM_CS) && PS.MEM_V){
        dcache_access(PS.MEM_ADDRESS, &read_word, wdata, &dcache_r, w0, w1);
    }
    ...
    if(PS.MEM_V){
        if(Get_UNCOND_OP(PS.MEM_CS)){
            ...
            if((PS.MEM_CC & 0x4) && (PS.MEM_IR & 0x0800)){
                mem_pcmux = 1;
            }
            else if((PS.MEM_CC & 0x2) && (PS.MEM_IR & 0x0400)){
                mem_pcmux = 1;
            }
            else if((PS.MEM_CC & 0x1) && (PS.MEM_IR & 0x0200)){
                mem_pcmux = 1;
            }
            else{
                mem_pcmux = 0;
  ...
  /* your code for AGEX stage goes here */
    ...
    switch(Get_ADDR2MUX(PS.AGEX_CS))
    {
        case 0:
            ...
            break;
        case 1:
            adder2 = sext(PS.AGEX_IR & 0x3F, 6);
            break;
        case 2:
            adder2 = sext(PS.AGEX_IR & 0x1FF, 9);
            break;
        case 3:
            adder2 = sext(PS.AGEX_IR & 0x7FF, 11);
            break;
    }
    ...
    if(Get_ADDRESSMUX(PS.AGEX_CS)){
        address = Low16bits(adder1 + adder2);
    }
    else{
    ...
    switch(Get_ALUK(PS.AGEX_CS)){
        case 0:
            alu_result = Low16bits(aluA + aluB);
            break;
        case 1:
            alu_result = Low16bits(aluA & aluB);
            break;
        case 2:
            ...
            break;
        case 3:
            alu_result = Low16bits(aluB);
            break;
    }
    //shifter
    int shiftdir = (PS.AGEX_IR & 0x0030) >> 4;
    int shiftamt = (PS.AGEX_IR & 0x000F);
    int shfout;
    //lshf
    if(!(shiftdir & 0x0003)){
        shfout = PS.AGEX_SR1 << shiftamt;}
    //rshfl
    else if(shiftdir == 0x0001){
        int temp = PS.AGEX_SR1 >> shiftamt;
        //mask out the bits that were shifted out
        int mask = 0xFFFF << (16 - shiftamt);
        mask = Low16bits(mask);
        temp = temp & ~mask; 
        temp = Low16bits(temp);
        shfout = temp;
    ...
    else if(shiftdir == 0x0003){
        int temp = PS.AGEX_SR1 >> shiftamt;
        int mask = 0xFFFF << (16 - shiftamt - 1);
        mask = Low16bits(mask);
        temp = temp & ~mask;
...
void DE_stage() {
  ...
  int ii, jj = 0;
    ...
    int cs = CONTROL_STORE[CONTROL_STORE_ADDRESS][0];

    //reg access
    int sr1val, sr2val, sr1, sr2;
    ...
    if(Get_DRMUX(cs)){
        ...
        drid = (PS.DE_IR & 0x0E00) >> 9;
    }



    //v logic and ld agex logic
    int LD_AGEX, valid; 
    dep_stall = 0;
    if(!PS.DE_V){
        if(!mem_stall){
    ...
    else{ //perform dependency check
        if(Get_SR1_NEEDED(cs)){
            if(v_agex_ld_reg){
                if(sr1 = PS.AGEX_DRID) dep_stall = 1;
            }
            if(v_mem_ld_reg){
                if(sr1 = PS.MEM_DRID) dep_stall = 1;
            }
            if(v_sr_ld_reg){
                if(sr1 = PS.SR_DRID) dep_stall = 1;
            }
        }
        if(Get_SR2_NEEDED(cs)){
            if(v_agex_ld_reg){
                if(sr2 = PS.AGEX_DRID) dep_stall = 1;
            }
            if(v_mem_ld_reg){
                if(sr2 = PS.MEM_DRID) dep_stall = 1;
            }
            if(v_sr_ld_reg){
                if(sr2 = PS.SR_DRID) dep_stall = 1;
            }
        }
        if(Get_DE_BR_OP(cs)){
    ...
    //stall logic
    if(PS.DE_V) v_de_br_stall = Get_DE_BR_STALL(cs);
...
void FETCH_stage() {
  ...
  /* your code for FETCH stage goes here */
    ...
    //gen ld_de, valid, ld_pc 
    int ld_de, valid, ld_pc;
    if(mem_stall || dep_stall){
        ld_de = 0;
        ld_pc = 0;
    }
    else if(v_de_br_stall || v_agex_br_stall || v_mem_br_stall){
        ld_de = 1;
        valid = 0;
